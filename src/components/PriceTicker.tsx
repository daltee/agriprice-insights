  ...
  useEffect(() => {
    supabase
      ...
      .then(({ data }) => {
        ...
        setTicks(data.map((d: any) => ({ produce: d.produce?.name ?? "", market: d.market?.name ?? "", price: Number(d.price) })));
      });
  }, []);
  ...
  return (
    <div className="bg-charcoal text-gold overflow-hidden border-y border-charcoal/50">
      <div className="ticker-track py-3">
        {items.length === 0 ? (
          <span className="px-8 text-sm tracking-wide opacity-70">No prices submitted yet — be the first.</span>
        ) : items.map((t, i) => (
